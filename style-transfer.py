...
import cv2
def createVideoStyle(videoSourceFile, videoOutputFile, styleName="hubble_pillars_of_creation", frameRate=30.0, frame_size=1080):
    cap = cv2.VideoCapture(videoSourceFile)

    fourc = cv2.VideoWriter_fourcc(*'mp4v')
    print(videoOutputFile)
    out = cv2.VideoWriter(videoOutputFile, fourc, frameRate, (frame_size,frame_size))
 
    print(f"Using style_name={styleName}")
    counter = 0
    while True:
        print(f"{counter}")
        ...
        if flag:
            ...
            outputs = hub_module(tf.constant(resized_image_np), tf.constant(style_images[styleName]))
            stylized_image = outputs[0]
        ...
        counter = counter + 1
        # just to do quick tests
#         if counter >=200:
#             break
    out.release()

for styleName in style_urls.keys():
  createVideoStyle(r"resources/cropped/nicole-dance-segmentCROP-trim.mp4", r"results/hubble2.mp4", styleName=styleName)
